$(document).ready(function () {
    ...
    $.ajax({
        type: 'GET',
        url: "http://127.0.0.1:8000/check-auth",
        dataType: 'json',
        success: function (authResponse) {
            console.log(authResponse)
            if (!authResponse.isAuthenticated) {
                // User is not authenticated
                $('.box-comment-system.comment').html('<h5>Bạn cần đăng nhập <a href="http://127.0.0.1:8000/login" target="_blank">Login</a></h5>');
            } else {
                var formDefault = '<h1>Comments</h1>' +
                    ...
                    '<input type="hidden" name="user_id" value="' + authResponse.user.id + '">' +
                    '<input type="hidden" name="parent_id" value="0">' +
                    '<label for="commentName" class="form-label fw-bold commentName mb-2 fs-6">' + authResponse.user.name + '</label>' +
        ...
        success: function (response) {
            ...
            if (response.success) {
                response.comment.forEach(function (comment) {
                    renderComment(comment,response.user);
                });
    ...
    function renderComment(comment,user) {
        ...
        var newCommentHTML =
            ...
            '<p>' + comment.content + '</p>' +
            '<button class="reply-btn btn btn-outline-primary ' + (user.name === undefined ? 'd-none' : '') + '" data-parent-id="' + comment.id + '" style="font-size: 10px;">Reply</button>' +
            '</div>' +
            ...
            '<label for="commentName" class="form-label fw-bold commentName mb-2 fs-6">'+user.name+'</label>' +
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    var newCommentHTML =
                        ...
                        '</div>' +
                        '<div class="box-content">' +
                        '<label for="commentName" class="form-label fw-bold mb-2 fs-6">' + response.user[0].name + '</label>' +
                        '<span class="m-0 ps-2" style="font-size: 12px;">' + timeAgo + ' </span>' +
                        ...
                        '</div>' +
                        '<div class="form-content w-100">' +
                        '<input type="hidden" name="user_id" value="' + response.user[0].id + '">' +
                        '<input type="hidden" name="parent_id" value="' + response.comment.id + '">' +
                        '<label for="commentName" class="form-label fw-bold commentName mb-2 fs-6">' + response.user[0].name + '</label>' +
                        '<input type="text" class="form-control commentText" name="content" id="commentText" placeholder="Enter your reply">' +
                        '<div class="d-flex justify-content-end align-items-center my-2">' +
                        '<button type="submit" class="btn btn-primary"> Submit </button>' +
                        '</div>' +
